"""LangGraph workflow for chronos-agent."""
...
def _route_after_parse(state: AgentState) -> str:
    return "resolve_ambiguity" if state.get("clarification_needed") else "plan_tools"


def _route_after_plan(state: AgentState) -> str:
    return "human_confirm" if state.get("requires_confirmation") else "execute"


def _route_after_confirm(state: AgentState) -> str:
    actions = state.get("planned_actions", [])
    return "execute" if actions else "respond"


def build_graph(llm: LLMProvider | None = None, checkpointer=None):
