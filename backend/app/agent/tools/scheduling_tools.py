...
async def find_free_slots(
    ...
    """Find free slots of the requested duration, respecting office hours."""
    cfg = config or {}
    ...
    busy = sorted(
        [(e.start_at, e.end_at) for e in events if not e.all_day],
        key=lambda x: x[0],
    )
    ...
    while day <= end_date:
        ...
        cursor = datetime.combine(day, datetime.min.time()).replace(
            hour=office_start_hour, tzinfo=UTC
        )
        day_end = cursor.replace(hour=office_end_hour)

        day_busy = [
            (max(s, cursor), min(e, day_end))
            for s, e in busy
            if e > cursor and s < day_end
        ]
        day_busy.sort()
